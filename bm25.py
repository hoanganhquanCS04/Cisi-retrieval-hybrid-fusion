...
def score_choices_with_local_bm25(tokenized_context, tokenized_choices, k1=1.5, b=0.75, epsilon=0.25):
    ...
    if not avg_doc_len:
        return [0.0] * len(tokenized_choices)
    ...
    needed_terms = set()
    for choice_terms in tokenized_choices:
        needed_terms.update(choice_terms)
    ...
    return [
        sum(
            query_tf * term_scores.get(term, 0.0)
            for term, query_tf in Counter(choice_terms).items()
        )
        for choice_terms in tokenized_choices
    ]
...
def select_answer(question_text, choices, context_text):
    ...
    scores = score_choices_with_local_bm25(tokenized_context, tokenized_choices)
    best_choice = "A"
    max_score = -1
    for key, score in zip(valid_keys, scores):
        if score > max_score:
            max_score = score
            best_choice = key
